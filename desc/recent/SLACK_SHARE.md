...
```
🎚️ 04 · 옵션 최대 선택 개수 (옵션 그룹에서 메뉴별 입력)
옵션 그룹 ✏️ 수정 모달의 「메뉴별 최대 선택 개수」 섹션에서 연결된 메뉴마다 항목별 횟수 설정.
...
```
📥 09 · 엑셀 다운로드 버튼 통일
화면마다 다른 라벨(📤 내보내기 · 📤 엑셀로 받기 · 📥 엑셀·CSV 받기)을 「📥 엑셀 다운로드」 하나로 통일. 모달에서 엑셀(.xlsx) / CSV(.csv) 형식 선택 가능.
